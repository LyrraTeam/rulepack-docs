...
writes the agent files.

```bash title="terminal"
rulepack add @ichi/nextjs-strict
rulepack add nextjs-strict          # house packs install by bare name
rulepack add @ichi/pack --force     # overwrite existing files without prompting
```

## init

Scaffold a `rulepack.json` in the current directory. Discovers existing agent
files and asks which agents to target.
