...
A pack's source repo holds the agent rule files and a root `rulepack.json`. The
`agents` map decides which files ship; `files` globs decide what else rides
along (a README, shared snippets).
...
tarball, verifies its SHA-256 against the registry header, and writes the agent
